          ...
          <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-7">
            <div>
              ...
              <input id="username" type="text" formControlName="username" autocomplete="username"
                     class="brand-input-lg" placeholder="Ingrese su usuario" />
            ...
            <div>
              ...
              <div class="relative">
                <input id="password" [type]="showPassword() ? 'text' : 'password'"
                       ...
                       class="brand-input-lg pr-12" placeholder="Ingrese su contraseña" />
                <button type="button" (click)="togglePassword()"
                        class="absolute inset-y-0 right-4 my-auto text-[var(--brand-700)]/80 hover:opacity-90"
                        aria-label="Mostrar/ocultar contraseña">
                  <img *ngIf="!showPassword()" src="assets/icons/visibility.svg" alt="Mostrar contraseña"
                       class="w-5 h-5" />
                  <img *ngIf="showPassword()" src="assets/icons/visibility_off.svg" alt="Ocultar contraseña"
                       class="w-5 h-5" />
                </button>
              </div>
            ...
            <p *ngIf="errorMsg()" class="mt-3 text-sm text-red-600">
              {{ errorMsg() }}
            </p>
          </form>
        </div>
...
export class LoginPage {
  ...
  onSubmit() {
    ...
    // mock simple → reemplaza luego por Keycloak o tu backend
    const ok = this.auth.login(
      this.form.value.username ?? '',
      this.form.value.password ?? ''
    );

    // Simula latencia mínima para UX
    setTimeout(() => {
